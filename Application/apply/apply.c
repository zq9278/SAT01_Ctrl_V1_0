...
volatile u16 PressureSet=30000;//????Ú…?5000-40000
...
void SendFrame(uint8_t seq, uint16_t response_id, const uint8_t *data, uint8_t data_len)
{
    ...
    // ???? CRC??Modbus??
    uint16_t crc = ModbusCRC16(&tx_buffer[3], len - 2);  // ???CRC¦Ë????
        ...
        case 0x0001:
            ...
            break;
				...
				case 0x0002:
					...
					RightTempSet=DefultTemperature+50*frame->payload[2];//????????Ú…
					LeftTempSet=DefultTemperature+50*frame->payload[3];//????????Ú…
